...
多视图学习框架（基于GMAE）:同时拥有分类头（分类，通过增加分类头输出与标签的损失实现）和kmeans（聚类，同时测试分类头和聚类头的输出的评价指标）
...
```shell
terminal output:
2025-12-30 22:46:11,053 - D:\BaiduSyncdisk\MvAE-ContrastExperiment\A-HaveDone\MvAE(ours)\GMAE-Github\classification.py - INFO - {'epoch': 10, 'acc': 0.9525, 'nmi': 0.9111247474488984, 'ari': 0.908272859749846, 'pur': 0.9525}
  5%|▌         | 10/200 [00:26<09:59,  3.15s/it]
Epoch 10
+---------+-----------+--------+--------+--------+
| Split   | Method    |  ACC   |  NMI   |  PUR   |
+---------+-----------+--------+--------+--------+
| Train   | CLS       | 0.9700 | 0.9289 | 0.9700 |
| Train   | KMeans    | 0.9613 | 0.9141 | 0.9613 |
+---------+-----------+--------+--------+--------+
| Test    | CLS       | 0.9525 | 0.9111 | 0.9525 |
| Test    | KMeans    | 0.9550 | 0.9207 | 0.9550 |
+---------+-----------+--------+--------+--------+
...
聚类头：网络学习到更好的表征，聚类头用kmeans处理输出簇（如果是多视图聚类就不用区分train和test）
```
